...
console.log("PROGRAM: [59] - [FORM VALIDATION WITH REGEX AND DOM ] ");
...
document.addEventListener('DOMContentLoaded', function() { 
    ...
    function validateField(field) { 
        ...
        if (value) { 
            ...
            if (rules.matchWith) { 
                const matchField = document.getElementById(rules.matchWith); 
                if (matchField && value !== matchField.value) { 
                    errors.push(rules.message); 
                } 
            } 
             
            // Custom validation for credit card (Luhn algorithm) 
    ...
    function updateFieldUI(field, errors) { 
        ...
        if (errors.length === 0 && fieldState.get(field.id).touched) { 
            ...
            if (errorContainer) errorContainer.textContent = errors[0]; 
        } else { 
            if (successIcon) successIcon.style.display = 'none'; 
            if (errorIcon) errorIcon.style.display = 'none'; 
            if (errorContainer) errorContainer.textContent = ''; 
    ...
    function calculatePasswordStrength(password) { 
        if (!password) return { score: 0, label: 'No password', color: '#ddd' }; 
        ...
        form.addEventListener('submit', function(e) { 
            ...
            if (validateForm()) { 
                ...
                if (successMessage) { 
                    ...
                    successMessage.textContent = '✗ Please fix validation errors'; 
                } 
            } 
        }); 
    } 
