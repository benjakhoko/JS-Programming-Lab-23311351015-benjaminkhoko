...
console.log("PROGRAM: [57] - [POPULATING A TABLE WITH JAVASCRIPT (COMPREHENSIVE DOM EXAMPLE)  ] ");
...
'benjaminkhoko@gmail.com', major: 'Computer Science' }, 
...
email: 'fiona@example.com', major: 'Mathematics' } 
    ...
    const studentForm = studentFormContainer ? studentFormContainer.querySelector('form') : null; 
...
${student.grade.charAt(0).toLowerCase()}">${student.grade}</span></td
> 
                <td>${student.email}</td> 
                <td>${student.major}</td> 
...
title="Delete">âœ•</button> 
    ...
    function updateStats() { 
        ...
        const avgAge = total > 0 ? (filteredData.reduce((sum, s) => sum + 
s.age, 0) / total).toFixed(1) : 0; 
        const aGrades = filteredData.filter(s => s.grade.startsWith('A')).length; 
        const csCount = filteredData.filter(s => s.major === 'Computer Science').length; 
...
: ''} data-page="prev">«</button>`; 
        ...
        for (let i = 1; i <= totalPages; i++) { 
            if (i === 1 || i === totalPages || (i >= currentPage - 2 && i <= 
currentPage + 2)) { 
                html += `<button class="page-btn ${i === currentPage ? 
'active' : ''}" data-page="${i}">${i}</button>`; 
            } else if (i === currentPage - 3 || i === currentPage + 3) { 
                html += '<span class="page-ellipsis">...</span>'; 
            } 
        }      
        // Next button 
        html += `<button class="page-btn" ${currentPage === totalPages ? 
'disabled' : ''} data-page="next">»</button>`; 
    ...
    function filterData() { 
        ...
        filteredData = currentData.filter(student => { 
            ...
            // Grade filter 
            const matchesGrade = !gradeValue || student.grade === 
gradeValue; 
             
            // Major filter 
            const matchesMajor = !majorValue || student.major === 
majorValue; 
...
b.firstName.localeCompare(a.firstName)); 
            ...
            case 'age-desc': 
                ...
                break; 
            case 'grade-asc': 
                filteredData.sort((a, b) => a.grade.localeCompare(b.grade)); 
                break; 
            case 'grade-desc': 
                filteredData.sort((a, b) => b.grade.localeCompare(a.grade)); 
                break; 
        } 
    } 
    ...
    function showForm(student = null) { 
        if (!studentFormContainer) return; 
        ...
        studentFormContainer.scrollIntoView({ behavior: 'smooth' }); 
    } 
    ...
    function saveStudent(formData) { 
        ...
        if (editingId) { 
            // Update existing 
            const index = currentData.findIndex(s => s.id === editingId); 
            if (index !== -1) { 
            ...
            // Add new 
            const newId = Math.max(...currentData.map(s => s.id), 0) + 1; 
            student.id = newId; 
    ...
    function exportToCSV() { 
        const headers = ['ID', 'First Name', 'Last Name', 'Age', 'Grade', 'Email', 
'Major']; 
        const rows = filteredData.map(s => [ 
            s.id, s.firstName, s.lastName, s.age, s.grade, s.email, s.major 
        ]); 
    ...
    if (tableBody) { 
        tableBody.addEventListener('click', function(e) { 
            ...
            if (target.classList.contains('edit-btn')) { 
                ...
                const student = currentData.find(s => s.id === id); 
